...
def send_post(post: dict, channel: str) -> bool:
    ...
    # Если нет ни текста ни картинок — видео или документ
    # Отправляем просто уведомление со ссылкой на оригинал
    ...
    # Картинки не скачались — шлём только текст
    if not images_bytes:
        return tg("sendMessage", chat_id=CHAT_ID, text=text, parse_mode="HTML") is not None
    ...
    if len(images_bytes) == 1:
        ok = send_photo_file(images_bytes[0], caption)
    else:
        ok = send_media_group_files(images_bytes, caption)

    # Текст не влез в caption — шлём отдельным сообщением
